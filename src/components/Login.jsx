      ...
      <div className="ff-login-card">
        ...
        {view === 'recovery' ? (
          <form onSubmit={handleRecoverySubmit}>
            <div className="mb-3">
              <label className="ff-form-label">Correo para recuperar contraseña</label>
            ...
            {recoveryError && (
              <div className="alert alert-danger py-2 d-flex align-items-center gap-2" role="alert">
                <i className="bi bi-exclamation-circle-fill" /><small>{recoveryError}</small>
              </div>
            )}

            {recoveryMessage && (
              <div className="alert alert-success py-2 d-flex align-items-center gap-2" role="alert">
                <i className="bi bi-check-circle-fill" /><small>{recoveryMessage}</small>
              </div>
            )}

            <button type="submit" className="btn btn-ff-primary w-100 py-2 fw-600 mt-1" disabled={recovering}>
              {recovering ? <><span className="spinner-border spinner-border-sm me-2" />Enviando...</> : <><i className="bi bi-send me-2" />Enviar instrucciones</>}
            </button>

            <button type="button" className="btn btn-outline-secondary w-100 mt-2" onClick={backToLogin}>
          ...
          <>
            ...
            <div className="text-center mt-3">
              <button type="button" className="btn btn-link p-0" onClick={openRecovery}>
